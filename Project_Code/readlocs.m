...
chanformat(1).description  = [ 'Polhemus native coordinate file containing scanned electrode positions. ','User must select the direction ','for the nose after importing the data file.' ];
...
chanformat(2).description  = [ 'BESA spherical coordinate file. Note that BESA spherical coordinates ','are different from Matlab spherical coordinates' ];
...
chanformat(3).description  = [ 'Standard 3-D cartesian coordinate files with electrode labels in ','the first column and X, Y, and Z coordinates in columns 2, 3, and 4' ];
...
chanformat(4).description  = [ 'Standard BESA 3-D cartesian coordinate files with electrode labels in ','the first column and X, Y, and Z coordinates in columns 2, 3, and 4.','Coordinates are re-oriented to fit the EEGLAB standard of having the ','nose along the +X axis.' ];
...
chanformat(7).description  = [ 'Neuroscan polar .asc file, automatically recentered to fit EEGLAB standard','of having ''Cz'' at (0,0).' ];
...
chanformat(8).description  = [ 'Neuroscan 3-D cartesian .dat file. Coordinates are re-oriented to fit ','the EEGLAB standard of having the nose along the +X axis.' ];
...
chanformat(9).description  = [ 'ASA .elc 3-D coordinate file containing scanned electrode positions. ','User must select the direction ','for the nose after importing the data file.' ];
...
chanformat(10).description  = [ 'EEGLAB file containing polar, cartesian 3-D, and spherical 3-D ','electrode locations.' ];

chanformat(10).importformat = { 'channum' 'labels'  'theta' 'radius' 'X' 'Y' 'Z' 'sph_theta' 'sph_phi','sph_radius' 'type' };
...
listcolformat = { 'labels' 'channum' 'theta' 'radius' 'sph_theta' 'sph_phi','sph_radius' 'sph_theta_besa' 'sph_phi_besa' 'gain' 'calib' 'type','X' 'Y' 'Z' '-X' '-Y' '-Z' 'custom1' 'custom2' 'custom3' 'custom4' 'ignore' 'not def' };
...
g = finputcheck( varargin,{ 'filetype'	   'string'  {}                 '';
...
if isstr(filename)
   ...
   if ~isempty(g.filetype) & ~strcmpi(g.filetype, 'custom')& ~strcmpi(g.filetype, 'asc') & ~strcmpi(g.filetype, 'elc') & ~strcmpi(g.filetype, 'dat')
   ...
   elseif strcmp(lower(g.filetype(1:end-1)), 'polhemus') | strcmp(g.filetype, 'polhemus')
       ...
       while isempty(array{indexbeg,1}) | (isstr(array{indexbeg,1}) & array{indexbeg,1}(1) == '%' )
